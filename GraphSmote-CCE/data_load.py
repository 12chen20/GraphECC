...
def load_data_citeseer(path="data/citeseer/", dataset="citeseer"):
    """Load citation network dataset (citeseer only for now)"""
    # input: idx_features_labels, adj
    # idx, labels are not required to be processed in advance
    # adj: save in the form of edges. idx1 idx2
    # output: adj, features, labels are all torch.tensor, in the dense form
    ...
    labels = np.array([classes_dict.get(label) for label in labels])

    # # build graph

    idx = np.arange(len(idx_features_labels[:, 0]))
    idx_map = {j: i for i, j in enumerate(idx_features_labels[:, 0])}

    edges_unordered = np.genfromtxt("{}{}.cites".format(path, dataset), dtype=np.dtype(str))
    edges = np.array([np.array([idx_map.get(idx, -1) for idx in edge]) for edge in edges_unordered])

    
    # Remove any rows with negative indices
